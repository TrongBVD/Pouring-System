...
int sessionProfileId = 4;      // Mặc định lưu profile số 4 (360ml)static const int CALIB_ID = 1;
...
bool webStopRequested = false;  // Thêm dòng
...
void startWifiAPWindow() {
  if (!wifiApRunning) {
    WiFi.mode(WIFI_AP);
    bool ok = WiFi.softAP(WIFI_NAME, WIFI_PASS, 1, 0, 8);
...
void queueWebStopAfterPump() {
  sessionCupPresentForPayload = true;
  queueSessionUpload("REMOTE_APP");  // Trả về lý do là bấm trên Web
}
...
void updatePumpRemovalConfidence(uint32_t now) {
  ...
  bool loadcellLooksRemoved =
    (weightG <= CUP_REMOVE_G_PUMP) || (weightGFiltered <= CUP_REMOVE_G_PUMP);

  bool ultraLooksRemoved =
    (!lastValid) || (lastDistanceCm > ULTRA_REMOVE_MAX_CM) || (lastDistanceCm < ULTRA_REMOVE_MIN_CM);
...
bool confirmedCupRemovedDuringPump() {
  bool bothConfirmed =
    (pumpLoadcellLowCount >= LOADCELL_REMOVE_CONFIRM_PUMP && pumpUltraLostCount >= 1) || (pumpLoadcellLowCount >= 1 && pumpUltraLostCount >= ULTRA_REMOVE_CONFIRM_PUMP);

  bool oneSensorVeryStrong =
    (pumpLoadcellLowCount >= SINGLE_SENSOR_HARD_CONFIRM) || (pumpUltraLostCount >= SINGLE_SENSOR_HARD_CONFIRM);
...
void resetSession() {
  ...
  isStopRequested = false;  // Bổ sung reset cờ Stop
...
void setup() {
  ...
  server.on("/pour", HTTP_POST, []() {
    ...
    float parsedMl = 0.0f;
    float parsedMl = 0.0f;
    if (server.hasArg("target_ml")) {
      parsedMl = server.arg("target_ml").toFloat();
    }
    requestedWebMl = parsedMl;

    // === THÊM ĐOẠN NÀY ĐỂ ĐỌC PROFILE_ID TỪ JAVA ===
    if (server.hasArg("profile_id")) {
      sessionProfileId = server.arg("profile_id").toInt();
      Serial.print("[DEBUG] ESP32 NHAN DUOC PROFILE_ID: ");
      Serial.println(sessionProfileId);
    } else {
  ...
  if (now - lastWeightMs >= weightPeriod) {
    ...
    if (accept && hasAcceptedWeight) {
      ...
      if (delta > MAX_STEP_CHANGE_IDLE_G) {
        if (pendingLargeStepCount == 0 || fabs(newWeight - pendingLargeStepWeightG) > LARGE_STEP_CONFIRM_BAND_G) {
    ...
    case ST_WAIT_3S:
      {
        ...
        // BỔ SUNG: Kiểm tra cả nút cứng VÀ nút STOP trên Web
        if (buttonPressedEvent || isStopRequested) {
          isStopRequested = false;
          Serial.println("[STOP] Cancel in WAIT_3S (By Button or Web)");
          resetSession();
          lcdClearSafe();
          stableStartSinceMs = 0;
          waitWeightSum = 0.0f;
        ...
        if ((now - stateSinceMs) >= PRE_POUR_DELAY_MS) {
          ...
          // === BẮT ĐẦU SỬA TỪ ĐÂY ===
          // Xác định thể tích cần rót: Nếu có lệnh web thì lấy số web, không thì lấy hằng số mặc định trừ đi ly rỗng
          float volumeToPour = (requestedWebMl > 0) ? requestedWebMl : (TARGET_FINAL_WEIGHT_G - 250.0f);

          targetWeightG = startWeightAvgG + volumeToPour;

          requestedWebMl = 0.0f;  // Đặt lại về 0 sau khi xài xong

          currentMeasuredWeightG = startWeightAvgG;

          // Lưu số ml mục tiêu vào payload để gửi lên backend lưu DB
          targetPourMlForPayload = volumeToPour;
    ...
    case ST_PUMPING:
      {
        ...
        // BỔ SUNG 1: Kiểm tra lệnh STOP từ Web
        if (isStopRequested) {
          isStopRequested = false;
          webStopRequested = true;
          pouring = false;
          pumpOff();
          Serial.println("[WEB] User finished pour via STOP API");
          goState(ST_SETTLING);
          break;
        }

        // BỔ SUNG 2: Kiểm tra mất kết nối WiFi nếu người dùng bắt đầu từ App
        if (strcmp(sessionStartReason, "REMOTE_APP") == 0 && !hasClientConnected()) {
          userStopRequested = true;
          pouring = false;
          pumpOff();
          Serial.println("[SAFETY] Client disconnected during remote pour! Auto Stop.");
          goState(ST_SETTLING);
          break;
        }
    ...
    case ST_SETTLING:
      {
        ...
        if ((now - stateSinceMs) >= SETTLE_AFTER_PUMP_MS) {
          ...
          if (webStopRequested) {
            queueWebStopAfterPump();  // Gọi hàm trả về "WEB_BUTTON"
            finishToDone(now, "Dung qua Web", "Lay ly ra");
